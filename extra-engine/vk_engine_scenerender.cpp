...
void VulkanEngine::execute_compute_cull(VkCommandBuffer cmd, RenderScene::MeshPass& pass,CullParams& params )
{
	...
	//1.build descriptor set (source access interface),cull compute need source data
	...
	//2.initialize cull compute params, binding descriptor set to compute pipeline and push constance
	...
	//from gpu re-write debug info to cpu
...
void VulkanEngine::ready_mesh_draw(VkCommandBuffer cmd)
{
	...
	//1. ready for render scene object
	//prepare for upload object data to gpu
	if (_renderScene.dirtyObjects.size() > 0)
	{
		...
		//re-upload all objects is copying data to object data buffer
		//update dirty objects is re-write data to 
		if (_renderScene.dirtyObjects.size() >= _renderScene.renderables.size() * 0.8)
		{
			...
			//map all renderable object and fill all objects to new buffer
			...
			//process storage dirty Objects ->upload dirty objects to buffer
			...
			//word size -> the number of wordtype occupied by an object
			...
			uint32_t launchcount = static_cast<uint32_t>(_renderScene.dirtyObjects.size() * wordsize);
			{
				...
				{
					//GPU object data from renderable array by dirty object handle index
					_renderScene.write_object(objectSSBO + i, _renderScene.dirtyObjects[i]);
					//find dirty object location in renderable objects array
					//dst zone -> renderable objects array,
					//offset start point -> renderable objects array head
					//dst offset  -> current GPUobjectData start point in renderable array
					//|----------------->|dst_offset
					//[obj0,obj1,obj2,...,obj.handle,...................]
					//					 |<-------->|wordsize
					uint32_t dstOffset = static_cast<uint32_t>(wordsize * _renderScene.dirtyObjects[i].handle);

					for (int b = 0; b < wordsize; b++ )
					{
						uint32_t tidx = dstOffset + b;//object memory index by sizeof(uint32_t)
						targetData[sidx] = tidx;
						sidx++;
					}
				}
				launchcount = sidx;//dirtyObject array occupied uint32_t count
			}
			unmap_buffer(newBuffer);
			...
			//one group size == 256,x group count = (launchcount/256)+1
	...
	for (int p = 0; p < 3; p++)
	{
		...
		{
			reallocate_buffer(pass.drawIndirectBuffer, pass.batches.size() * sizeof(GPUIndirectObject), VK_BUFFER_USAGE_TRANSFER_SRC_BIT |VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT, VMA_MEMORY_USAGE_GPU_ONLY);
		}
	...
	for (int p = 0; p < 3; p++)
	{
		...
		//if the pass has changed the batches(cmd,objID,batchID), need to reupload them
		if (pass.needsIndirectRefresh && pass.batches.size() > 0)
		{
			ZoneScopedNC("Refresh Indirect Buffer", tracy::Color::Red);
			//newbuffer = direct buffer ->VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT
			//drawIndirectBuffer
		...
		if (pass.needsInstanceRefresh && pass.flat_batches.size() >0)
		{
			...
			//Copy buffer need save buffer data,we should create barrier
	...
	//After pass async finish refelsh indirect array and instance array
	//can un map the new buffer(GPUindirectObject,GPUintance)
	for (auto b : unmaps)
	{
		unmap_buffer(b);
	}
	//binding all barriers include ready for scene and pass
...
void VulkanEngine::draw_objects_forward(VkCommandBuffer cmd, RenderScene::MeshPass& pass)
{
	...
	//light source info color,direction and power(color.w)
	...
	//push data to dynmem
	//--scene-data-offset------->|
	//---------------------------|--scene-parameters--|--camera-parameters----
	//-------------camera-data-offset---------------->|
	//scene_data_offset -> scene parameters start point in *mapped memory
	uint32_t scene_data_offset = get_current_frame().dynamicData.push(_sceneParameters);
	//std::cout << " After push forward scnen data, dynamicData current offset:" << get_current_frame().dynamicData.currentOffset << std::endl;

	//camera_data_offset -> camera parameters start point in *mapped memory
	uint32_t camera_data_offset = get_current_frame().dynamicData.push(camData);
	//std::cout << " After push forward camera data, dynamicData current offset:" << get_current_frame().dynamicData.currentOffset << std::endl;

	VkDescriptorBufferInfo objectBufferInfo = _renderScene.objectDataBuffer.get_info();

	VkDescriptorBufferInfo sceneInfo = get_current_frame().dynamicData.source.get_info();
	sceneInfo.range = sizeof(GPUSceneData);
	//std::cout << " forward scnenInfo.range:" << sizeof(GPUSceneData) << std::endl;

	VkDescriptorBufferInfo camInfo = get_current_frame().dynamicData.source.get_info();
	camInfo.range = sizeof(GPUCameraData);
	//std::cout << " forward cameraInfo.range:" << sizeof(GPUCameraData) << std::endl;
	...
	//std::cout << "forward scene_data_offset:" << scene_data_offset << std::endl;
	//std::cout << "forward camera_data_offset:"<<camera_data_offset << std::endl;
...
void VulkanEngine::draw_objects_shadow(VkCommandBuffer cmd, RenderScene::MeshPass& pass)
{
	...
	camData.proj = projection;
	camData.view = view;
	camData.viewproj = projection * view;//from camera space to light space
	
	//push data to dynmem
	//camera_data_offset -> camera data start point
	uint32_t camera_data_offset = get_current_frame().dynamicData.push(camData);
	//std::cout << " After push shadow camera data, dynamicData current offset:" << get_current_frame().dynamicData.currentOffset << std::endl;

	VkDescriptorBufferInfo objectBufferInfo = _renderScene.objectDataBuffer.get_info();

	VkDescriptorBufferInfo camInfo = get_current_frame().dynamicData.source.get_info();
	camInfo.range = sizeof(GPUCameraData);
	//std::cout << "shadow cameraInfo.range:" << sizeof(GPUCameraData) << std::endl;
	...
	std::vector<uint32_t> dynamic_offsets;
	dynamic_offsets.push_back(camera_data_offset);
	//std::cout << "shadow camera_data_offset:" << camera_data_offset << std::endl;
	execute_draw_commands(cmd, pass, ObjectDataSet, dynamic_offsets, GlobalSet);
}
...
void VulkanEngine::reduce_depth(VkCommandBuffer cmd)
{
	...
	for (int32_t i = 0; i < depthPyramidLevels; ++i)
	{
		...
		//destTarget.imageLayout = VK_IMAGE_LAYOUT_UNDEFINED;
		...
		if (i == 0)
		{
			...
			//sourceTarget.imageLayout = VK_IMAGE_LAYOUT_UNDEFINED;
		}
		...
		//a group has 32 parallel stream
		vkCmdDispatch(cmd, getGroupCount(levelWidth, 32), getGroupCount(levelHeight, 32), 1);

		//Configure and bind the barrier, 
		//in order to wait for the shader to fully write the data to the cuurent level image
		//prepare for shader read image in next level
		VkImageMemoryBarrier reduceBarrier = vkinit::image_barrier(_depthPyramid._image, VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT,VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_ASPECT_COLOR_BIT);
		//VkImageMemoryBarrier reduceBarrier = vkinit::image_barrier(_depthPyramid._image, VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_ASPECT_COLOR_BIT);
	...
	//Configure and bind the barrier, 
	//in order to wait for the shader to fully write the data to the entry depth pyram image
