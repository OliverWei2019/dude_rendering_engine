...
namespace vkutil {
	...
	VkDescriptorSetLayout DescriptorLayoutCache::create_descriptor_layout(VkDescriptorSetLayoutCreateInfo* info)
	{
		...
		if (it != layoutCache.end())
		{
			...
			//std::cout << "Descriptor builder***********************create dptorSetLayout" << std::endl;
			//layoutCache.emplace()
			//add to cache
	...
	void DescriptorLayoutCache::cleanup()
	{
		...
		uint32_t dptorSetLayoutCount = 0;
		uint32_t VaildDptorSetLayoutCount = 0;
		for (auto pair : layoutCache)
		{
			if (pair.second) {
				vkDestroyDescriptorSetLayout(device, pair.second, nullptr);
				VaildDptorSetLayoutCount++;
			}
			dptorSetLayoutCount++;
		}
		std::cout << " Descriptor set layout Cache has " << dptorSetLayoutCount
			<< " descriptor set layout, cleanup " << VaildDptorSetLayoutCount
			<< " vaild descriptor set layouts" << std::endl;
	}
